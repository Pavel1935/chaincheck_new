...
REDIS_KEY_MOC = "ea:pashkarob@gmail.com"
...
def get_verification_code_moc(email=None, retries=None, delay=None) -> str:
    """Пытается получить код подтверждения из Redis с повторными попытками"""
    redis_conn = get_redis_connection()
    ...
    for attempt in range(retries):
        code = redis_conn.get(REDIS_KEY_MOC if email is None else f"ea:{email}")
        if code:
            print(f"[Redis] Код из Redis: {code}")
            return code
        time.sleep(delay)

    pytest.fail(f"Код не найден в Redis по ключу {REDIS_KEY_MOC} после {retries*delay} секунд")
